...
- **L34 SEO / AEO / GEO Discoverability** (Band 5, v2.22) — overwhelmingly **actionable technical/content fixes** (robots, schema, rendering, answer-first blocks, evidence density) → normal queue, ranked by tier-leverage (a Tier-0 break that caps all three funnels outranks a Tier-3 polish). It *hands off* rather than re-files: CWV depth → L17, content voice → L26, social unfurl → L20, share-of-AI-voice-vs-competitors → L24 (that competitive read is the only piece that may land in the strategic bucket). Dedups against L20 on OG/meta and L26 on readability-level SEO.
