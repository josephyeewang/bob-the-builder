...
| **L31 Input & Data-Flow Trace** | submit a real input via Playwright / curl, then **query the DB and each consumer** to confirm the field actually arrived and is correct; **census each field's at-rest non-null rate** (`count(field IS NOT NULL)/count(*)` — a ~0% column is a dead axis, v2.31); **spot-check collapse/group-by ratios** (`count(*)` vs `count(distinct key)` per source — ~10:1 where you expected ~1:1 = a blend bug); scan attribute columns for malformed values (a numeric id in a `ticker`/symbol column); try registering the same email twice (dedup); fire concurrent redemptions to probe atomicity / double-spend; CodeQL / Semgrep taint queries; `grep`/LSP find-references for every reader of a field | schema / migrations (intended storage); the spec's intended fan-out per field | "is this field *supposed* to reach that consumer?" — resolved against spec, not invented |
| **L32 Analytical Method Soundness** | run the algorithm across an easy/typical/hard/adversarial input spread and **inspect intermediate values** (weights, chosen branch, triggered default) — not just final output; flip one input and check whether output moves (dead-input detection); force missing/contradictory/out-of-range inputs and observe degradation; **empirical-validity forensics (v2.31):** for any "no signal / anti-predictive" verdict run a **split-half** (does first-half performance predict second-half? — selection-on-outcome fakes nulls); for "one dominant driver / monolithic" **residualize the top factor** and re-rank; for "too sparse / untestable" **query the RAW volume** first (an upstream collapse/backfill bug fakes sparsity); test convergence/confirmation **non-monotonically** | the method's code/prompt; the spec's *claimed* depth; the domain anchor (guideline/literature) | domain-correctness certification ("would a clinician stand behind this method?") → route to L27 / human expert |
| **L33 Output Register & Audience Fit** | **generate 10-30 real outputs** from the running product across input cases; run `textstat`/Hemingway for Flesch-Kincaid reading-grade on each; programmatic jargon/acronym scan against a domain term list; check answer-first / labeled-structure conformance via parsing | the spec's declared audience + house-style target; output-generating prompts/templates | final register/tone judgment for the intended reader (NN/g tone dimensions) is partly subjective — execution gathers the samples + metrics, human/Claude scores fit |
| **L34 SEO / AEO / GEO Discoverability** | run **Lighthouse / PageSpeed** (perf+SEO scores); run a **crawler** (LibreCrawl / open-seo-crawler / CrawlIQ / Screaming Frog) for crawl/index/link analysis; `curl` robots.txt + AI-crawler directives + status codes; **compare raw `curl` HTML vs. Playwright-rendered DOM** (is content client-rendered?); validate JSON-LD via **Rich Results Test**; **spot-prompt ChatGPT/Perplexity/AI-Overviews** with 5-10 category questions and record citations+sentiment; pull share-of-AI-voice from Profound/Otterly/Semrush-AI if available | the spec's target queries/markets; existing schema in source; `llms.txt` | backlink/domain authority needs paid data (Ahrefs/Semrush) — flag, don't fake; AI-visibility snapshots are volatile (40-60% monthly churn) — human judgment integrates trend |
