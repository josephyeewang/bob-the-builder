...
| **L31 Input & Data-Flow Trace** | submit a real input via Playwright / curl, then **query the DB and each consumer** to confirm the field actually arrived and is correct; try registering the same email twice (dedup); fire concurrent redemptions to probe atomicity / double-spend; CodeQL / Semgrep taint queries; `grep`/LSP find-references for every reader of a field | schema / migrations (intended storage); the spec's intended fan-out per field | "is this field *supposed* to reach that consumer?" — resolved against spec, not invented |
| **L32 Analytical Method Soundness** | run the algorithm across an easy/typical/hard/adversarial input spread and **inspect intermediate values** (weights, chosen branch, triggered default) — not just final output; flip one input and check whether output moves (dead-input detection); force missing/contradictory/out-of-range inputs and observe degradation | the method's code/prompt; the spec's *claimed* depth; the domain anchor (guideline/literature) | domain-correctness certification ("would a clinician stand behind this method?") → route to L27 / human expert |
