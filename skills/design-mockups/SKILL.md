...
python ~/.claude/skills/design-mockups/mockup.py custom jobs.json  # [{name,prompt,refs?}] — refs = paths of liked frames to style-transfer
python ~/.claude/skills/design-mockups/mockup.py gallery       # rebuild the consolidated searchable gallery of out_dir
```
Every run also writes `_contact.png` (contact sheet) and `gallery-ALL.html` (one searchable, named board — open it and give feedback BY NAME).
...
6. **Build the winner** — hand the winning comp + a DESIGN.md to a coding agent; reuse the 4K backdrop as a real asset; implement motion with GSAP + Lenis + Motion.dev.
...
## Files
- `mockup.py` — CLI. `lib/gen.py` — config-driven generator. `lib/archetypes.py` — 44 core + 16 cinematic + 14 modern archetypes + 12 art-direction STYLES. `lib/gallery.py` — contact sheet + consolidated searchable gallery. `lib/vid.py` — Veo motion previews. `config.example.json` — per-project config template.
