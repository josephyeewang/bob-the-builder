...
**Announce up front (NEW mode):** when NEW mode is selected, tell the user in one plain line that **there is a deliberate Pre-Build Review Gate (Step 6.5) before any code is written** — a machine audit of the whole plan plus a human review-and-tweak checkpoint — so they know a real stop-and-review is coming and won't feel rushed from spec into build (Rule 22).
...
22. **Never cross from design into build without the Pre-Build Review Gate (v2.28).** The spec→code boundary (Step 6.5) is a HARD stop requiring **both** a fresh-context machine audit of the whole doc-set/plan **and** an affirmative human review-and-sign-off — *before the first line of product code.* This is non-negotiable and applies even when momentum is high, the user said "proceed/build," or the specs feel done. Three reasons it's its own rule: (a) it's the least-reversible transition (foundation mistakes compound through every later phase); (b) the post-build hardening audits (N+1) are too late to catch a bad foundation or a mis-sequenced plan; (c) a non-engineer needs one explicit, plain-language chance to review/tweak the entire plan before code exists. **Announce this gate to the user UP FRONT** — at mode selection / Step 1 — so they know a deliberate review checkpoint is coming before any building. A generic "continue" earlier in the conversation is NOT gate approval; the gate requires its own affirmative review. *(Origin: the InsiderIntent dogfood — Bob slid straight from spec into building with no intentional stop, no machine audit, and no human recap; the founder had to catch it and call the audit himself. That is exactly the failure this rule prevents.)*

23. **Living-doc coherence is a *triggered, mechanical* sweep — not a manual audit the human has to call (v2.29).** A living spec set drifts every time a **shared identifier** that is restated across docs changes: a **version number**, **phase ID** (P-n), **decision ID** (D-n), **capability/subsystem code** (C-n/S-n), or **section anchor** (§). Two halves. **(a) Propagate-in-the-same-edit:** when you change a shared identifier, update *every* reference to it in the same commit — index/secondary docs (CLAUDE.md, PROJECT-STATE, manifests, "synced through D-n" lines) lag first and get re-read last, so they are where drift hides. **(b) Sweep on a trigger, mechanically-first:** run a **Coherence Sweep** automatically — on every living-doc version bump, every new decision-log entry, every phase/code renumber, and always as the first lens of the Pre-Build Gate (Step 6.5a lens 1) — and run `scripts/coherence-check.sh <docs-dir>` *before* reasoning so the human never hand-greps. The regression classes (the EMBT/DLL drift taxonomy): **(1)** version-string drift across docs; **(2)** phase/code renumbers not fully propagated; **(3)** dangling §/D cross-references; **(4)** decision-log contiguity (no gaps/dupes) + highest-ID matches every "synced through" claim; **(5)** retired-vocabulary residue in *live* (non-changelog) sections; **(6)** point-in-time/secondary docs reading as live (need a "📸 snapshot — not a living doc" banner). The script *harvests + flags the mechanical classes*; the AI/human judges the semantic rest. *(Origin: the InsiderIntent dogfood — two consecutive sessions (D-073, then D-074) each needed a **manual** full cross-doc sync audit, and the second caught drift the first missed. Pull-based gates weren't enough: coherence must be push-based and tooled, or the non-engineer becomes the drift detector.)*

24. **The Build-Integrity Loop — milestone audits are push, not pull; the build never forgets an earlier step or decision (v2.30).** Bob already has the *pieces* — per-phase Reconcile, the Phase Gate, the Capability Traceability Matrix (CTM), the decision-log, the A7 lens cadence, the Rule-23 coherence sweep. The failure is that they are **advisory and human-pulled**, so the founder keeps having to *call* the audit, and earlier capabilities/decisions silently drift. This rule makes the cadence **enforced, push-based, and decision-faithful**, tracked in an **`audit-ledger.json`** (what audit ran, when, against which commit) that **gate-blocks**: a phase/gate cannot advance until its required audits ran or are **explicitly deferred with a logged reason**.
    - **(a) The trigger → audit map (push, not pull).** Each trigger auto-fires its audit set; Bob runs them *before* asking to advance, never waiting to be told:
      - *Every living-doc version bump / decision-log entry / phase-or-code renumber* → the **mechanical coherence sweep** (Rule 23 / `coherence-check.sh`, now also scanning `contracts/` + non-`.md` SSOT).
      - *Every phase boundary (start + end)* → the **Anti-Forgetting Gate** (b) + a **scoped lens panel** (the Curated panel, now mandatory not advisory — §A7).
      - *Every named gate (spec-complete · Pre-Build 6.5 · pre-launch)* → the **heavy audits**: the **Independent Audit Panel re-fired** (effectiveness — "given modern AI, is this still the right capability set?", not only at first spec draft), the **Rule-20 expert-demand frame checklist**, the **real-world benchmark** (Rule 20 frame), and the **full multi-agent coherence audit**.
    - **(b) The Anti-Forgetting Gate (the heart of "don't forget earlier steps/decisions").** At every phase boundary, mechanically re-verify three things and surface any miss as a stop-condition:
      1. **CTM-diff (capabilities):** every Product-Spec capability still maps to exactly one phase and still has a home; a capability that was planned and is now **missing / unbuilt / dropped without a logged decision** = a *forgotten-capability* alarm.
      2. **Decision-honor (decisions):** every still-live core decision in `decision-log.md` is **re-checked against the current build + docs** — a phase that silently violates an earlier binding decision (e.g. "no LLM numbers", "off-the-shelf-first", "bidirectional symmetry") is caught here, not three phases later. (The coherence sweep checks *doc-internal* consistency; this checks *build-honors-decisions*.)
      3. **Finding propagation (changes flow backward too):** generalize the class-level bug scan to **all** findings — a finding or change in phase N must be scanned against earlier phases (does it retrofit them?) and either fixed or logged with a revisit trigger. No finding dies in the phase that found it.
    - **(c) Tooling makes the mechanical part zero-touch.** `coherence-check.sh` runs as a **git pre-commit hook** (wired by `bob-init`) and gains a `--build-integrity` mode (CTM completeness · decision-log refs resolve · capability-without-a-home · SSOT/`contracts/` drift). The script *harvests + gate-fails on the mechanical classes*; the AI/human judges the semantic rest (decision-honor, effectiveness, the benchmark). *(Origin: the InsiderIntent build — across one session the founder had to manually trigger three coherence audits (D-073/074/078), an effectiveness panel, a robustness sweep, and a real-world benchmark; every one of them should have fired automatically at a milestone. Pull-based review means the non-engineer is the scheduler — and a capability or decision dropped between phases is invisible until someone re-reads everything.)*

25. **Green ≠ Correct — the silent-failure forensics a data/analysis audit must actually perform (v2.31).** Rule 24 makes audits *fire*; this rule defines what they must *catch*. CI passing, a health scan at "N/N", and "the job ran" verify *known-incident classes + syntactic success* — NOT semantic correctness, data completeness, or analytical soundness; "green" and a clean-looking table both sit happily on corrupted inputs and mis-specified methods. For any data-backed product, run these forensics **proactively** (don't wait for the founder to push) and treat every plausible headline adversarially — *"assume I'm wrong; what did I miss?"*:
    - **Census input null-rates at rest** — the actual non-null % of every field, not "it can be written." A ~0% column is a **DEAD axis** silently producing nothing downstream (→ L31 §4b).
    - **Check every collapse/dedup/group key captures all varying entities** — a key omitting a varying entity **silently blends** distinct records; spot-check the collapse ratio (~10:1 where you expected ~1:1 = a blend bug — → L31 canonical defects).
    - **Distrust "no signal / anti-predictive"** drawn from selecting-on-realized-outcome (regression-to-mean) → demand a **bias-free test** (split-half / out-of-sample — → L32 §4b).
    - **Distrust "too sparse / untestable"** → verify **RAW volume** first (usually an upstream collapse/backfill bug, not a data fact — → L32 §4b).
    - **One dominant factor = peak-seeking** → **residualize** it out and hunt orthogonal signals before declaring a single "king" (→ L32 §4b).
    - **Test convergence/confirmation signals non-monotonically** (the edge often peaks at moderate breadth and decays when crowded); and **compute the actual numbers before shipping anything outward-facing.**
    - **Census LOADED-N against the SOURCE's own `count(*)`, per partition (v2.33)** — null-rate census assumes you *pulled* the data; also verify you didn't silently *truncate* it. A paginator capped at 1000 rows/window, a stray `LIMIT`, or a recency floor makes every downstream "green" a result computed on a *fraction*. Assert `loaded ≈ source count` per source/type before trusting anything; keyset-page to exhaustion. (Origin: InsiderIntent D-277 — every recent machine ran on ~25% of events; insider_buy was 12.7k of 52.5k.)
    - **Main-effect vs best-surviving-cell (v2.33)** — a conditioned cohort ("A×B×C looks great") is a **conditioning artifact** if the *unconditioned* main effect of its ingredients is flat/negative. Print each factor's main effect beside its winning cell; a +24% survivor built from three individually-negative factors is in-sample selection, not signal. Pair with: **de-dupe survivors by stat-signature** (majority-value riders re-count the same cell → inflated counts), require OOS to **span ≥K non-overlapping time-windows** (a 90%-hit cell that is 13 rows in one 2-month episode is noise), **residualize trailing momentum/crowding** (not just sector), and confirm every derived time-window feature is **backward-only** from decision time (a ±window silently peeks ahead). (Origin: InsiderIntent D-277 — a "convergence" lead reproduced exactly yet its main effect was −6.5%/37%-hit and lived in one Jan-Feb window; the machine now flags all four inline.)
    Deepened in **L31** (dead-column census · aggregation-blend · malformed-value passthrough) and **L32 §4b** (empirical-validity forensics), and made PERPETUAL in **L37 (v2.32)** — Rule 25 makes the forensics fire *once*; L37 makes them fire *forever* as a standing, generative crawler that rotates the whole data-fidelity chain and turns each new catch into a reactive invariant. *(Origin: InsiderIntent D-267→271 — nearly every founder push uncovered a real defect all the green checks missed: an issuer-blind collapse bug blending 13F portfolios, 100%-null swept columns, a selection-artifact "track-record is anti-predictive" null, a peak-seeking "monolithic" finding, and a source with no backfill path read as "too sparse." Rule 24 ensures the audit fires; Rule 25 ensures it catches the silent-failure class — so the non-engineer isn't the linchpin.)*

26. **Keep the machine hermetic & refactor by invariant — the ENGINEERING silent-failures (v2.33).** Rule 25 catches *data/analysis* silent-failures; this is their *code/plumbing* twin — the class that turns "works on my machine / the build is green" into a CI or production break. Enforce proactively on any non-trivial refactor, consolidation, or module move:
    - **Imports must be side-effect-free.** A module that reads env / hits the network / touches the DB *at import time* passes locally (your `.env.local` is present) and dies in hermetic CI — a `KeyError` at pytest *collection*, before a single test runs. Read creds/config **lazily**, inside the function that needs them. (Origin: D-277 — a guard module's import-time `os.environ[...]` would have turned the CI gate red on the next push.)
    - **Refactor against a REPRODUCIBILITY INVARIANT.** Before moving/renaming/rewiring, capture a known deterministic output (a survivor count, a row total, a hash). After EACH step — not just at the end — re-run and assert it is **bit-identical**. A consolidation that changes the numbers silently changed behavior. (Origin: D-277 — "90d raw=137 / sr=322" held a 6-step core consolidation honest.)
    - **Moving a shared module is a CLASS-LEVEL change.** Grep EVERY importer — including `package.json` scripts, CI workflows, and OTHER pipelines, not just the one you're refactoring. A live consumer you can't migrate now gets a thin **compat shim** at the old path; never leave it import-broken. (Origin: D-277 — relocating a shared db/price util into `core/` silently broke a platform backtester wired in `package.json`.)
    - **Know the DB statement-timeout traps** (Postgres/PostgREST): exact `count(*)` on a big filtered table times out → `count=estimated`; `eq.<col> + order-by-<unindexed>` times out → keyset-page on an INDEXED column. A 500 from a "simple" query is usually a plan/index problem, not your logic.
    - **Be honest about parallel speedups.** Process-parallel pandas/groupby over large data is **memory-bandwidth-bound** — more cores ≈ flat. Measure and report the real number (a truthful 1.5× beats a claimed 8×); reach for a vectorized/numpy inner loop before more processes.
    - **The patchwork→one-machine shape** (when N one-off scripts have accreted, an EVOLVE-mode staple): converge on ONE dataset builder → ONE factor/config catalog the engine reads (delete inline copies) → **guards as a pipeline invariant** (the completeness check runs at BUILD and at USE, not bolted onto one script) → an append-only **results ledger** (endpoint tracking: "what did we test & what happened") → a **config-driven CLI** (a non-engineer iterates by editing a config, never the code) → archive the scratch. Verify the invariant at each step. (Origin: D-277 — ~100 scripts → a 7-module `core/` machine.)
...
6.5. **🛑 PRE-BUILD REVIEW GATE (v2.28 — MANDATORY, the single most important gate; NEVER skip — Rule 22).** The transition from *design* (all spec docs) to *build* (first line of product code) is the highest-stakes, least-reversible boundary in the whole protocol — yet it's the one most easily skated past when momentum builds. **Stop here, deliberately, every time.** Two parts, in order:
   - **6.5a — Machine audit (fresh-context, multi-lens).** Run an independent audit panel over the COMPLETE document set + plan *before* any code — do NOT wait for the post-build hardening audits (those are too late to catch a bad foundation). Minimum lenses, cast by archetype (Appendix K) but always including: **(1) internal consistency & drift** (names/IDs/versions/dropped-concepts/contradictions across all docs — the EMBT/DLL regression class), **(2) foundation-first sequencing** (is the build order laying a clean foundation, or rushing to a visible surface / MVP-for-its-own-sake?), **(3) completeness/table-stakes-in-the-plan** (auth/billing/security/privacy/ops present in the plan even if built later), **(4) data-quality/integrity** (for any data product: is ingest/processing pressure-tested + self-checking, not assumed-correct?), **(5) domain-expert POV** (the archetype's expert seats), and **(6) AI-nativity** (L36) if AI-forward. Aggregate → a written findings doc + a triaged fix list. Apply the clear-win fixes; surface the judgment-calls.
   - **6.5b — Human recap & sign-off (`→ HG`, HARD).** Bob presents a **plain-language recap for the non-engineer**: what the product *is*, what will be built and in what order (the phase plan), the key decisions made (and any reversed), the open decisions needing the user, and the machine-audit's top findings + what was changed. Then Bob explicitly invites: *"This is your review-and-tweak gate before we write code — anything to change, cut, add, or resequence?"* **Do NOT proceed to Step 7 until the user has actively reviewed and approved.** Approval must be affirmative, not assumed from silence or from a generic "continue."
...
|---|---|---|
| 1. Engineering Foundation | L01–L06, **L31**, **L37** | Code work, match spec, security / privacy / supply-chain? (L01 includes prior A7a-A7j Hygiene + Liveness — Knip / Schemathesis / Playwright / Vitest / promptfoo) **L31 (v2.21):** trace one input/field end-to-end — stored / deduped / propagated to every consumer / terminal states resolved? **L37 (v2.32):** the STANDING, generative data-fidelity crawler — perpetually rotates the whole SOURCE→…→SERVE chain, tests falsifiable hypotheses vs INDEPENDENT oracles, and emits a new reactive invariant per confirmed defect (the perpetual complement to L31's one-shot trace). |
...
A7 can be re-invoked anytime; standard cadence = a Curated panel after each major phase + a Full Enchilada before launch. **As of v2.30 (Rule 24) this cadence is ENFORCED, not advisory** — the Curated panel + the Anti-Forgetting Gate auto-fire at each phase boundary and gate-block advancement (tracked in `audit-ledger.json`); the human no longer has to remember to call them.
...
*Core Reference for Build Protocol v2.33 — 2026-08-04*
*v2.33: Rule 26 — Keep the machine hermetic & refactor by invariant (the ENGINEERING silent-failures), the code/plumbing twin of Rule 25's data forensics: imports must be side-effect-free (import-time env/IO reads pass locally, die at hermetic-CI collection → read creds lazily); refactor against a REPRODUCIBILITY INVARIANT (capture a deterministic output, assert bit-identical after EACH step, not just at the end); moving a shared module is CLASS-LEVEL (grep every importer incl package.json/CI/other pipelines; shim live consumers, never leave import-broken); the DB statement-timeout traps (count=estimated over exact; keyset on indexed cols over eq+order-by-unindexed); honest parallel speedups (pandas/groupby is memory-bandwidth-bound — report the real 1.5×, vectorize before adding processes); the patchwork→one-machine consolidation shape (one builder → one config/factor catalog → guards as a pipeline invariant → results ledger → config-driven CLI → archive scratch, invariant-checked each step). Rule 25 ALSO extended with two D-277 catches: census LOADED-N vs source count(*) per partition (silent pagination/LIMIT truncation → analysis on ~25% of data), and main-effect-vs-best-surviving-cell (a conditioned winner whose ingredients' unconditioned main effects are negative = in-sample selection artifact — de-dupe by stat-signature, require ≥K non-overlapping OOS windows, residualize momentum/crowding, backward-only derivations). Origin: InsiderIntent D-273→277 — a session that consolidated ~100 accreted scripts into one guarded/ledgered/parallel machine and, along the way, hit each of these silent-failure classes; the fixes are build-agnostic, so they live here.*
*v2.32: L37 — Perpetual Data-Fidelity Crawler (+ companion `audit-lenses/_data-fidelity-chain.md`). Rule 25's silent-failure forensics fire once per audit; L37 makes them PERPETUAL — a standing, scheduled, generative crawler that walks the SOURCE→EXTRACT→INTERPRET→LINK→COLLAPSE→OUTCOME→SERVE chain, rotates the whole surface via a coverage ledger, GENERATES falsifiable fidelity hypotheses each run and tests them against INDEPENDENT oracles (value↔shares×price, implied-price, null-census, collapse-ratio, coverage-depth, cross-source reconciliation, distribution drift, code-vs-data temporal consistency, selection-bias/pseudo-replication re-tests — 13 in the companion), and on each confirmed defect emits a NEW reactive invariant back into the incident-encoded registry (imagination → memory). Reframe: the reactive fidelity registry is MEMORY (replays known failures); L37 is IMAGINATION (hunts unseen ones) — removing the human as the generative hypothesis source. Lens count 36 → 37. Boundary honored (D-005): the emit auto-DRAFTS a new invariant + a lens retro; the human merges (Bob never auto-edits its own lenses). Origin: InsiderIntent D-247/D-267→272 — a ~two-dozen-session recurrence of the same fidelity-bug classes (13F ×1000 units, allFilers 1000-row clamp, 100%-null axes, congress-corpus floor, stale-pre-classifier ingest, issuer-blind collapse), each caught only because a human pushed.*
*v2.31: Rule 25 — Green ≠ Correct (silent-failure forensics). Rule 24 makes audits FIRE; Rule 25 defines what a data/analysis audit must CATCH — because CI/health-scan/"it ran" verify known-incident classes + syntactic success, not semantic correctness/completeness/soundness. Proactive forensics (don't wait for the founder to push): census input null-rates (a ~0% field is a DEAD axis); confirm every collapse/dedup/group key captures all varying entities (spot-check the collapse ratio — a blend bug); distrust "no signal/anti-predictive" from selecting-on-outcome (→ split-half) and "too sparse/untestable" (→ verify raw volume for an upstream collapse/backfill bug); residualize the dominant factor before declaring "one driver"; test convergence non-monotonically; compute real numbers before shipping outward-facing. Deepened in L31 (dead-column census · aggregation-blend · malformed-passthrough) + L32 §4b (empirical-validity forensics). Lens count unchanged (36). Origin: InsiderIntent D-267→271 — nearly every founder push uncovered a real defect all the green checks missed (issuer-blind collapse bug, 100%-null columns, a selection-artifact null, peak-seeking, an un-backfilled source).*
*v2.30: Rule 24 — the Build-Integrity Loop. Bob's review machinery (Reconcile, Phase Gate, CTM, decision-log, A7 cadence, the Rule-23 coherence sweep) was rich but ADVISORY/pull-based, so the founder kept having to call audits and earlier capabilities/decisions silently drifted. Rule 24 makes it ENFORCED + push: a trigger→audit map auto-fires at every doc-edit / phase-boundary / named-gate; an Anti-Forgetting Gate at each phase boundary mechanically re-checks (1) CTM-diff (no capability silently dropped), (2) decision-honor (build still honors every live decision-log decision), (3) finding-propagation (findings scanned backward against earlier phases); all tracked in `audit-ledger.json` that gate-blocks. Rule 20 gains a real-world-benchmark frame + "re-run at each milestone." `coherence-check.sh` gains a code/contract (class 7) scan — the .md-only sweep's blind spot (it immediately caught a stale §-ref + a dropped-dependency residue in a downstream project's contracts/) — and is wired as a git pre-commit hook by bob-init. Origin: the InsiderIntent build — one session needed the founder to manually trigger three coherence audits, an effectiveness panel, a robustness sweep, and a real-world benchmark; all should have auto-fired at milestones.*
*v2.29: Rule 23 + `scripts/coherence-check.sh` — living-doc coherence is a TRIGGERED, MECHANICAL sweep (on every version bump / decision-log entry / phase-or-code renumber, and as Pre-Build-Gate lens 1), not a manual audit the human has to call. The script harvests the mechanical drift classes (decision-log contiguity, version-string drift, dangling §/D cross-refs, "synced through D-n" lag, retired-term residue, missing snapshot banners); the AI/human judges the semantic rest. Origin: the InsiderIntent dogfood — two consecutive sessions (D-073, then D-074) each needed a MANUAL full cross-doc sync audit, and the second caught drift the first missed; pull-based gates weren't enough, so coherence is now push-based and tooled.*
*v2.28: Rule 22 + Step 6.5 — a MANDATORY Pre-Build Review Gate (fresh-context machine audit of the whole doc-set/plan + an affirmative human review-and-tweak sign-off) before the first line of product code, announced up front at mode selection. Origin: the InsiderIntent dogfood slid straight from spec into build with no intentional stop, no machine audit, and no human recap — the founder had to catch it. This gate makes that stop deliberate and unskippable.*
