...
### D-009: Perpetual Data-Fidelity Crawler — make the silent-failure forensics STANDING + generative (L37)

- **Date:** 2026-07-23
- **Status:** Accepted (v2.32)
- **Context:** The direct sequel to D-008. Rule 25 defined *what* a data/analysis audit must catch and deepened L31/L32 to catch it — but it still **fires once per audit**, and the InsiderIntent recurrence continued: the D-272 session alone surfaced a **13F ×1000 units bug** (a normalize cutoff set to 2024 when the SEC rule changed in 2023 → Berkshire AAPL stored at $177 **trillion**, with the value-sanity check *structurally excluding 13F* and a unit test *hardcoding the wrong cutoff*), an **`allFilers()` 1000-row PostgREST clamp** (the daily derivation lane silently ran 2 of 25 filers), **34 congress PUTs mislabeled bullish** (ingested one day before the classifier shipped), and **reconcile scans that grew past the DB statement timeout and hard-failed two production crons** — every one green until a human or a fresh audit found it. Root cause: the existing defense (`lib/dq/fidelity.ts`, ~23 named invariants) is **reactive by construction** — each invariant encodes a bug that *already* bit, so it can never catch the *next* class; meanwhile the data surface (fields/sources/joins/collapses/derivations/served reads) grows monotonically, leaving every new edge unmonitored. The human remained the **generative hypothesis source** ("wait, that can't be right"). Joe: *"why do we keep having fidelity/extraction/interpretation/linkage issues — design an audit that catches these AND goes beyond, that I can run continually so it crawls for new angles."*
- **Decision:** Add **L37 — Perpetual Data-Fidelity Crawler** (Band 1) + companion **`audit-lenses/_data-fidelity-chain.md`** (the 7-stage chain SOURCE→EXTRACT→INTERPRET→LINK→COLLAPSE→OUTCOME→SERVE, the 7-mode × stage failure taxonomy, and a **13-oracle catalog**). Split the problem into **memory vs imagination**: the reactive registry is memory (replay known failures every commit — keep it); L37 is imagination — a **STANDING, scheduled, generative** crawler that reads a coverage ledger, rotates to an un-crawled slice, **generates a falsifiable fidelity hypothesis per cell**, and **tests it against an INDEPENDENT oracle** (value↔Σ(shares×price), implied-price band, null-rate census, collapse-ratio, coverage-depth vs source truth, cross-source reconciliation, extraction-completeness diff, distribution drift, code-vs-data temporal consistency, selection-bias re-test, pseudo-replication clustering, served↔computed, row-count conservation). The oracle-selection heuristic prefers the truth-source *most independent* of the pipeline (recompute-from-parts / market price / the source's own counts beat any same-codebase range assert — the kind that already missed). **Self-learning (imagination → memory):** each confirmed defect auto-**drafts** a new reactive invariant for the registry + a lens retro. **Boundary honored (D-005):** it drafts; the human merges — Bob never auto-edits its own lenses. Rules unchanged; **lens library 36 → 37.** Wired across `skill/SKILL.md`, `build-protocol(-core).md`; recorded here. *(Note: L37's value is as a perpetual scheduled job — run it via the project's Cron→queue→worker lane, not only as a one-shot pre-launch pass.)*

### D-008: Green ≠ Correct — the silent-failure forensics a data/analysis audit must perform (Rule 25 / L31 + L32)

- **Date:** 2026-07-22
- **Status:** Accepted (v2.31)
- **Context:** The direct sequel to D-007. Rule 24 made Bob's audits **fire** automatically at milestones — but an audit that fires without knowing *what to look for* still misses defects. Across the InsiderIntent D-267→271 sessions, nearly **every** improvement was surfaced by a founder *push-back on a plausible-looking result*, and each push uncovered a real defect that CI (green), the `fidelity:scan` (23/23), and "the lab ran successfully" all reported as healthy: an **issuer-blind collapse key** blended each fund's whole 13F portfolio into one arbitrary-issuer row (793→47) and corrupted every downstream stat; **`move_5d` and `position_type` were 100%/near-100% NULL** on hundreds of thousands of rows (dead swept axes producing silent nothing); a **"track-record is anti-predictive" null** was a regression-to-the-mean *selection artifact* (a bias-free split-half showed skill actually persists); a **"findings are monolithic"** conclusion was *peak-seeking* (ranking by effect-size restated one dominant factor N times — residualizing surfaced a whole orthogonal tier); and **"13F too sparse to test lead-lag"** was an un-backfilled source (a backfill took it 794→8,289 and the question became testable-and-answered). Joe: *"I don't want to always be the linchpin to catch these — consolidate the learnings so the system catches them."* The gap: "green + it ran + a clean table" verifies *known-incident classes + syntactic success*, never semantic correctness, data completeness, or analytical soundness.
- **Decision:** Add **Rule 25 — Green ≠ Correct** (cross-cutting; the *what-to-catch* complement to Rule 24's *when-to-fire*), and deepen the two lenses that own the surfaces. **L31 (Input & Data-Flow Trace)** gains an at-rest **null-rate census** (step 4b — a ~0% column is a dead axis, not "it can be written"), and two canonical flow defects — **aggregation-key blend** (a collapse/dedup/group key omitting a varying entity silently merges records; spot-check the collapse ratio) and **malformed-value passthrough** (e.g. a CIK leaking into a `ticker` column). **L32 (Analytical Method Soundness)** gains **§4b Empirical-Validity Forensics**: selection-on-outcome false-nulls (→ split-half / out-of-sample), peak-seeking false-"monolithic" (→ residualize the dominant factor), sparse≠untestable (→ verify raw volume for an upstream collapse/backfill bug), and non-monotonic convergence/crowding — plus finding categories + an anti-pattern. The stance runs **proactively** (don't wait for the founder to push) and treats every headline adversarially. **No new lens** (deepens L31/L32); rules → 25; lens library unchanged at 36. Wired across `build-protocol(-core).md`, `CLAUDE.md`, `_execution-principle.md`, `skill/SKILL.md`, both READMEs; recorded under `evolutions/004-silent-failure-audit/`.
