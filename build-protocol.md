...
15. **Preserve the edge — don't let guardrails neuter the capability (v2.26).** A product's own rigor, safety/compliance caution, honesty rules, and example-lists can silently suppress the *differentiating capability* they exist to protect. Four facets: **(a) Layer.** Keep constraints at the **communication / legal / safety layer, not the analysis / capability layer** — the engine pursues its goal to the hilt; only *how results are phrased and to whom* is constrained. **(b) Open vocabularies.** Enumerated content examples (signals, categories, methods, styles) are seeds, not closed enums — instruct the system to seek beyond them; *governance* lists (safety/refusal/limits) stay closed and binding. **(c) Notability ≠ validation.** Don't let statistical/quality rigor built for *patterns* bury an individual *novel/rare/notable* instance — separate "is it worth surfacing?" (often assessable on one instance) from "is it validated?" (needs volume). **(d) Usefulness-as-felt.** Rigor that yields correct-but-useless output fails the user; guarantee felt value, with honesty as the framing, not as a reason to deliver nothing. The bright line for any guardrail: does it stop the engine *DOING* the valuable thing (a neuter — fix it, relocate to the communication layer) or only *SAYING* it carelessly/to the wrong audience (legitimate — keep it)? Audit with **lens L35 (Capability Preservation)**. *(Origin: the InsiderIntent NEW-mode build — the single most-recurring correction, surfaced ~5×; a human had to catch each.)*
...
1. **Internal consistency & drift** — names, capability IDs, version numbers, dropped/orphaned concepts, contradictions across all docs, spec↔code drift, decision-log integrity (the EMBT/DLL hallucination/mutation/regression class). **Run `scripts/coherence-check.sh <docs-dir>` first (Rule 23)** — it mechanically harvests this lens's regression classes (decision-log contiguity, version-string drift, dangling §/D cross-refs, "synced through D-n" lag, retired-term residue, missing snapshot banners) so the audit reasons over a clean harvest instead of hand-grepping. This same sweep is *also* a standing trigger during the spec phase (every living-doc version bump / decision entry / phase-or-code renumber), not only here at the gate.
...
**Guardrails (enforce, don't instruct):** git is the rollback, NOT `/rewind` (it can't undo shell deletes); "DO NOT delete X" in prose gets ignored under pressure (the Replit prod-DB-wipe incident) → put it in `deny` rules / isolation; **never expose PROD credentials to an autonomous run — separate prod/dev (the real Replit lesson, not "add DROP to the deny-list")**; the in-app Stop button is documented-unreliable for runaways, so the true kill-switch is `--max-turns` + a **server-side spend cap** (Anthropic Console → Limits — `ccusage` is a monitoring dashboard, NOT a brake) + container teardown. **The `deny` rules are belt-and-suspenders (prefix, not semantic — they miss `find -delete`, `> file`, `python rmtree`); the REAL destructive-class guardrail in `auto` mode is Claude Code's built-in classifier.** For host protection (not just code), use the official Bash sandbox `/sandbox` (Seatbelt/bubblewrap) with `failIfUnavailable: true` + `allowUnsandboxedCommands: false` (else it silently runs unsandboxed) and mirror credential paths in `permissions.deny Read(...)`; worktrees protect the code, only a sandbox/microVM protects the machine. **Where "swarm-like" parallelism actually helps: the verification side** (adversarial reviewers, drift-checks — read-heavy), not the write-heavy building.

**⚙️ The runbook (how a non-engineer actually runs a phase autonomously):**
1. **Branch first:** `git switch -c phase-N` — autonomous commits land here, never on `main`. (Rollback = `git switch main && git branch -D phase-N`, since `git reset --hard` is denied.)
2. **Turn auto ON — mind the trap:** `defaultMode: "auto"` is **IGNORED** in a project's `.claude/settings.json` (a repo can't grant itself auto). Enable it per-session via **Shift+Tab until the status line shows `⏵⏵ auto mode on`**, or set `"defaultMode":"auto"` in **`~/.claude/settings.json`** (user-global). Needs a recent model (Opus 4.6+/Sonnet 4.6+/Fable 5). Auto is NOT `--dangerously-skip-permissions`.
3. **Let it run to the milestone.** The green-gate Stop hook forces it to keep fixing until build+tests pass (it is loop-guarded — bounded retries, not just `stop_hook_active`, which is absent from current docs and buggy). Note the **ceiling:** auto pauses after **3 consecutive / 20 total** classifier blocks, and *aborts* in headless `-p` — so sequence phases to avoid the block-class; anything needing a blocked action (a real deploy/migrate) is a human checkpoint by construction.
4. **The reviewer must be INVOKED** — a subagent file on disk does nothing until called. At every phase boundary, the phase's Reconcile/gate step explicitly delegates to the `reviewer` subagent on the diff BEFORE requesting the human gate. (Optional stronger enforcement: GitHub-native mode — each phase = one issue → one PR that `Closes #N` and structurally can't self-merge; the PR is the hard gate.)
5. **You review the diff at the milestone** (Green≠Correct — the gate proves tests *ran*, not that they're *right*). Approve → next phase.
6. **Turn auto OFF (Shift+Tab → Manual) when you return to foundations/spec** — auto mode is session/global, not phase-aware.
...
| 5. Reach & Distribution | L17–L20, **L34** | Does it work on mobile, in other languages, for users with disabilities, and unfurl when shared? **L34:** will search / answer / generative engines find, rank, extract, and cite the site? (SEO / AEO / GEO, 4-tier funnel) |
...
|---------|------|---------|-------------|
| v2.31 | 2026-07-22 | **Rule 25 — Green ≠ Correct: the silent-failure forensics a data/analysis audit must actually perform.** Rule 24 makes audits *fire*; Rule 25 defines what they must *catch*. CI passing, a health scan at "N/N", and "the job ran" verify *known-incident classes + syntactic success* — NOT semantic correctness, data completeness, or analytical soundness; green + a clean-looking table both sit happily on corrupted inputs and mis-specified methods. For any data-backed product, run these forensics **proactively** (don't wait for the founder to push), treating every plausible headline adversarially: **census input null-rates at rest** (a ~0% field is a DEAD axis silently producing nothing — → L31 §4b); **confirm every collapse/dedup/group key captures all varying entities** (a key omitting a varying entity silently blends records — spot-check the collapse ratio; ~10:1 = a blend bug — → L31 defects); **distrust "no signal / anti-predictive"** from selecting-on-realized-outcome (regression-to-mean → split-half — → L32 §4b); **distrust "too sparse / untestable"** (→ verify RAW volume — usually an upstream collapse/backfill bug); **one dominant factor = peak-seeking** (→ residualize before declaring a "king"); **test convergence non-monotonically** (edge peaks at moderate breadth, decays when crowded); **compute real numbers before shipping outward-facing.** Deepened in **L31** (dead-column census · aggregation-blend · malformed-passthrough) + **L32 §4b** (empirical-validity forensics) + CLAUDE.md Key Rule. Lens count unchanged (36); rules → 25. | InsiderIntent D-267→271: nearly every founder push uncovered a real defect all the green checks missed — an issuer-blind collapse bug blending 13F portfolios (793→47), 100%-null swept columns (`move_5d`, `position_type` on 285k rows), a selection-artifact "track-record is anti-predictive" null (split-half showed it persists), a peak-seeking "monolithic" finding (residualizing surfaced a whole tier), and a source with no backfill path read as "too sparse" (a fix took 13F 794→8,289). Joe: *"I don't want to always be the linchpin to catch these."* |
| v2.30 | 2026-06-24 | **Rule 24 — the Build-Integrity Loop: milestone audits are push, not pull; the build never forgets an earlier step or decision.** Bob already had the pieces (Reconcile · Phase Gate · CTM · decision-log · A7 cadence · Rule-23 sweep) but they were **advisory/human-pulled**, so the founder kept *calling* audits and earlier capabilities/decisions silently drifted. Rule 24 makes the cadence **enforced + push-based**, tracked in `audit-ledger.json` that **gate-blocks**: **(a) trigger→audit map** — doc-edit→coherence sweep · phase-boundary→Anti-Forgetting Gate + mandatory Curated lens panel · named-gate→heavy audits (Independent Audit Panel re-fired for effectiveness · Rule-20 expert-demand checklist · real-world benchmark · full multi-agent coherence audit). **(b) Anti-Forgetting Gate** at every phase boundary: CTM-diff (no capability silently dropped) · decision-honor (build still honors every live decision-log decision) · finding-propagation (findings scanned backward against earlier phases). **(c)** `coherence-check.sh` gains a **class-7 code/contract scan** (the .md-only sweep's blind spot — immediately caught a stale `§2.4` + a dropped-`@ai-spine-core` residue in a downstream project's `contracts/` that every prior `.md`-only audit missed) and is **wired as a git pre-commit hook** by `bob-init`. **Rule 20** gains a **real-world-benchmark frame** ("find the best real example a practitioner would cite — match it or beat it?") + "re-run the frame checklist at each milestone, not once." | InsiderIntent build: across one session the founder had to manually trigger three coherence audits (D-073/074/078), an effectiveness panel, a robustness sweep, and a real-world benchmark — every one should have auto-fired at a milestone. Joe: *"how do we upgrade bob so these iterations happen automatically… do we have proper steps that ensure we build step-by-step, recursive audits at each milestone, propagate findings, and don't forget earlier steps or core decisions?"* |
| v2.29 | 2026-06-24 | **Rule 23 + `scripts/coherence-check.sh` — living-doc coherence is a TRIGGERED, MECHANICAL sweep, not a manual audit.** A living spec set drifts whenever a *shared identifier* restated across docs changes (version number, phase ID, decision ID, capability/subsystem code, section anchor §). Rule 23 has two halves: **(a)** propagate every shared-identifier change to all references in the *same* edit (index/secondary docs — CLAUDE.md, PROJECT-STATE, manifests, "synced through D-n" lines — lag first, get re-read last); **(b)** run a **Coherence Sweep** on a *trigger* — every version bump, every decision-log entry, every phase/code renumber, and as Pre-Build-Gate lens 1 — invoking `scripts/coherence-check.sh <docs-dir>` first so the human never hand-greps. The script (macOS bash-3.2-portable) harvests the mechanical drift classes: decision-log contiguity (gaps/dupes + highest-ID-vs-"synced-through" claims), version-string drift, §/D/P cross-ref inventory, retired-term residue (snapshot-doc + retirement-context filtered, review-list not hard-fail), and dated/secondary docs missing a "📸 snapshot" banner. Hard-fails only on contiguity gaps/dupes; everything else is a review list — the script harvests, the AI/human judges. Wired into Step 6.5a lens 1 + the core-ref footer. *(Changelog note: this table lags — rows v2.17–v2.28 live in the core-ref footer + `decision-log.md`, not back-filled here; that lag is itself the Rule-23 drift class, flagged for a future consolidation.)* | InsiderIntent dogfood: two consecutive sessions (D-073, then D-074) each needed a **manual** full cross-doc sync audit, and the second caught drift the first missed. Joe: *"seems like I'm manually doing a lot of checks and edits that should be part of the Bob NEW build process."* |
